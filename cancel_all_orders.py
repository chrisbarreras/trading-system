"""Cancel all open orders in Alpaca account."""
import os
...
def cancel_all_orders():
    ...
    for order in orders:
        ...
        print(f"Cancelling: {side} {qty} {symbol} (Order ID: {order_id})")
        ...
        if cancel_response.status_code == 200 or cancel_response.status_code == 204:
            print(f"  ✅ Cancelled successfully")
            cancelled += 1
        else:
            print(f"  ❌ Failed: {cancel_response.status_code} - {cancel_response.text}")
            failed += 1
...
if __name__ == "__main__":
    ...
    confirm = input("Are you sure you want to cancel ALL open orders? (yes/no): ")

    if confirm.lower() == "yes":
        print()
        cancel_all_orders()
    else:
        print("\nCancelled - no orders were modified")
